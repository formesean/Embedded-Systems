...
void main(void)
{
  ...
  for (;;)
  {
    // unsigned char input = PORTB;
    // unsigned char higherNibble = (PORTB >> 4) & 0x0F;
    // unsigned char lowerNibble = PORTB & 0x0F;

    // int msd = ((int)higherNibble) * 10;
    // int lsd = ((int)lowerNibble);
    // int num = msd + lsd;
    // PORTD = num;
